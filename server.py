...
def make_query(carrier: str, flight_number: Optional[int], date_of_origin: str, startStation: Optional[str], endStation: Optional[str]) -> Dict:
    """
...
async def _find_matching_doc_meta(query:dict,limit: int = 50)->dict:
    ...
    Returns: {"count": <int>, "document": [{doc_id, startStation, endStation, scheduledStartTime, seqNumber, flightStatus},...]}
    """
    try:
        _, _, col=await get_mongodb_client()
...
async def _get_document_by_id(doc_id:str,projection:dict=None)->Optional[dict]:
    try:
        ...
        #     oid = ObjectId(doc_id)
        # except Exception:
        #     logger.warning(f"Invalid ObjectId string provided: {doc_id}")
        #     return None
        result = await col.find_one({"_id":doc_id},projection or {})
        if result:
            result.pop("_id",None)
...
async def run_aggregated_query(
    ...
    Args:
        ...
        field: Field to aggregate, e.g. "flightLegState.pax.passengerCount.code".
