...
-For numeric summaries (counts,avg,sum,min,max) use run_aggregated_query.
-There are multiple flights with same flightnumber in a day, so always give the user the startStation and endStation to ask specific flight.
-For lists, flitered fetches, or flexible field selection use raw_mongodb_query.
-For single-flight lookups (carrier + flight_number + data) use get_flight_basic_info, get_operation_times, etc.
                ...
                try: 
                    if isinstance(resp,dict):
                        ...
                        if resp.get("ok") and isinstance(resp.get("data"),dict):
                            ...
                            candidate=resp
                        if isinstance(candidate,dict) and candidate.get("needs_route_selectioin"):
                            return {"plan":plan, "results": results, "need_route_selection":True}
                except:
                    pass
